...
describe('usePortfolioCalculations', () => {
  ...
  describe('Portfolio Totals', () => {
    it('should calculate total dollars correctly', () => {
      const { result } = renderHook(() => usePortfolioCalculations(mockPortfolio, toInvest));
    ...
    it('should handle empty portfolio', () => {
      const { result } = renderHook(() => usePortfolioCalculations([], createDollarAmount(1000)));
  ...
  describe('Account Calculations', () => {
    ...
    it('should calculate total for asset class in specific account', () => {
      ...
      expect(result.current.totalForAssetClassAccount('US Total Stock Market', '401k')).toBe(3000);
      expect(result.current.totalForAssetClassAccount('International Stocks', 'Roth IRA')).toBe(1000);
      expect(result.current.totalForAssetClassAccount('Bonds', 'Brokerage')).toBe(200);
    });

    it('should return zero for non-existent asset class or account', () => {
      const { result } = renderHook(() => usePortfolioCalculations(mockPortfolio, toInvest));

      expect(result.current.totalForAssetClassAccount('NonExistent', '401k')).toBe(0);
      expect(result.current.totalForAssetClassAccount('US Total Stock Market', 'NonExistent')).toBe(0);
    });
  });
  ...
  describe('Asset Class Calculations', () => {
    ...
    it('should handle zero total portfolio when calculating percentages', () => {
      ...
      const { result } = renderHook(() => usePortfolioCalculations(emptyPortfolio, createDollarAmount(0)));
  ...
  describe('Rebalancing Calculations', () => {
    ...
    it('should handle zero investment amount', () => {
      const { result } = renderHook(() => usePortfolioCalculations(mockPortfolio, createDollarAmount(0)));
    ...
    it('should handle case where asset class is over-allocated', () => {
      ...
      const { result } = renderHook(() => usePortfolioCalculations(overAllocatedPortfolio, createDollarAmount(1000)));
      ...
      expect(result.current.amountToBuy(overAllocatedPortfolio[1])).toBeGreaterThan(0);
      expect(result.current.amountToBuy(overAllocatedPortfolio[2])).toBeGreaterThan(0);
    });
    ...
    it('should handle case with no positive deltas', () => {
      ...
      const { result } = renderHook(() => usePortfolioCalculations(balancedPortfolio, createDollarAmount(0)));
  ...
  describe('Memoization', () => {
    it('should return stable function references', () => {
      const { result, rerender } = renderHook(() => usePortfolioCalculations(mockPortfolio, toInvest));
      ...
      expect(result.current.totalForAccount).toBe(firstRenderFunctions.totalForAccount);
      expect(result.current.totalForAssetClassAccount).toBe(firstRenderFunctions.totalForAssetClassAccount);
      expect(result.current.currentForAssetClass).toBe(firstRenderFunctions.currentForAssetClass);
      expect(result.current.currentPercentage).toBe(firstRenderFunctions.currentPercentage);
      expect(result.current.targetDollars).toBe(firstRenderFunctions.targetDollars);
      expect(result.current.amountToBuy).toBe(firstRenderFunctions.amountToBuy);
      expect(result.current.totalDollars).toBe(firstRenderFunctions.totalDollars);
    });
  });
});
