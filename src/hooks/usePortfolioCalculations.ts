...
export function usePortfolioCalculations(portfolio: AssetClass[], toInvest: number): UsePortfolioCalculationsReturn {
    ...
    const totalForAccount = useCallback((accountName: string): number => {
    ...
    const totalForAssetClassAccount = useCallback((assetClassName: string, accountName: string): number => {
    ...
    const targetDollars = useCallback((assetClass: AssetClass): number => {
        return portfolioTotals.totalWithInvestment * (assetClass.allocation / 100);
    }, [portfolioTotals.totalWithInvestment]);

    const amountToBuy = useCallback((assetClass: AssetClass): number => {
        if (toInvest <= 0) {
            return 0;
        }
    ...
    const totalDollars = useCallback((): number => portfolioTotals.totalDollars, [portfolioTotals.totalDollars]);
