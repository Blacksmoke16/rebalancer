...
export function useAssetClassForm({ portfolio, onPortfolioChange }: UseAssetClassFormProps) {
    ...
    const preserveFundValues = useCallback((newPortfolio: AssetClass[]): AssetClass[] => {
    ...
    const handleSubmit = useCallback((values: { portfolio: AssetClass[] }) => {
    ...
    const removeAssetClass = useCallback((index: number) => {
        form.removeListItem('portfolio', index);
    }, [form]);

    const addFund = useCallback((assetClassIndex: number) => {
    ...
    const removeFund = useCallback((assetClassIndex: number, fundIndex: number) => {
