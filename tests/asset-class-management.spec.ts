...
test.describe("Asset Class & Fund Management", () => {
  ...
  test("Edit Asset Class Allocation", async ({ page }) => {
    ...
    const usStockTargetField = page
      .getByTestId("asset-class-allocation-0")
      .last();
    ...
    await expect(
      page.getByRole("heading", {
        name: "Total Allocation: 95% (Must equal 100%)",
      }),
    ).toBeVisible();
  ...
  test("Delete Asset Class", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Delete entire US Total Bond" })
      .click();
    ...
    await expect(
      page.getByRole("heading", {
        name: "Total Allocation: 90% (Must equal 100%)",
      }),
    ).toBeVisible();
  ...
  test("Add Fund to Asset Class", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Delete fund VTSAX" }),
    ).toBeVisible();

    // 6. Verify both funds (VTI and VTSAX) are shown
    await expect(
      page.getByRole("button", { name: "Delete fund VTI" }),
    ).toBeVisible();
  ...
  test("Allocation Validation - Equals 100%", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Total Allocation: 100%" }),
    ).toBeVisible();
  ...
  test("Allocation Validation - Greater Than 100%", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: "Total Allocation: 110% (Must equal 100%)",
      }),
    ).toBeVisible();
